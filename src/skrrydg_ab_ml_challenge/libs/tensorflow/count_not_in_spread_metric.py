...
class CountNotInSpreadMetric(tf.keras.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        bid_not_skip_mask = fair_price < bid * (1 - 1e-4 * self.interest)
        ask_not_skip_mask = fair_price > ask * (1 + 1e-4 * self.interest)
        self.count_not_in_spread.assign_add(tf.math.reduce_sum(tf.cast(bid_not_skip_mask, tf.int32)))
        self.count_not_in_spread.assign_add(tf.math.reduce_sum(tf.cast(ask_not_skip_mask, tf.int32)))

        self.mask = tf.logical_or(bid_not_skip_mask, ask_not_skip_mask)

    def result(self):
        return self.count_not_in_spread
    
    def reset_state(self):
        self.count_not_in_spread.assign(0)